...
const FirstPage = () => {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:9999/api/job/all")
      .then((res) => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-blue-50 to-purple-50">
      ...
      <div className="w-full max-w-6xl mx-auto px-4 py-8 flex-grow">
        ...
        {/* 💼 If logged in — show job list or empty placeholder */}
        {isLoggedIn && (
          <>
            {jobList.length > 0 ? (
                ...
                {jobList.map((job, idx) => (
                  ...
                  >
                    ...
                    <p className="text-gray-600 text-sm mb-2">
                      {job.company.name} — {job.job.location.city}
                    </p>
                    <p className="text-gray-700 mb-3">{job.job.description}</p>
                    <p className="text-gray-500 text-sm mb-1">
                      Pay: ₹{job.payRange.min} - ₹{job.payRange.max}{" "}
                      {job.payRange.currency}
                    </p>
