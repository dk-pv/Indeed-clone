...
export default function JobDetailsPage2() {
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [userEmail, setUserEmail] = useState("user@example.com");
  ...
  useEffect(() => {
    ...
    const savedData = JSON.parse(localStorage.getItem("jobFormStep2"));
    if (savedData) {
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem("jobFormStep2", JSON.stringify(formData));
    setError("");
    navigate("/addJobDetails3");
  };
